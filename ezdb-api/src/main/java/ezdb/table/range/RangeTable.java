...
public interface RangeTable<H, R, V> extends Table<H, V> {
	 ...
	 * 
	 * @param hashKey  A key used group rows together.
	 * @param rangeKey A secondary key used to sort rows within the same hash key
	 *                 group.
	 ...
	 * @param rangeKey A secondary key used to sort rows within the same hash key
	 *                 group.
	 * @return The value, or null if the item does not match the hash/range pair.
	 */
	public V get(H hashKey, R rangeKey);

	/**
	 * Get the latest value for a given hash/range pair. It gets the overall latest.
	 * 
	 * @param hashKey A key used group rows together.
	...
	/**
	 * Get the latest value for a given hash/range pair. First it tries to get the
	 * value that is &lt;= rangeKey, if that fails it returns the value that is
	 * &gt;= rangeKey. If rangeKey is null, it gets the overall latest.
	 * 
	 * @param hashKey  A key used group rows together.
	 * @param rangeKey A secondary key used to sort rows within the same hash key
	 *                 group.
	 * @return The latest value, or null if the item does not match the hash/range
	 *         pair.
	 */
	 ...
	 * 
	 * @param hashKey A key used group rows together.
	 * @return An iterator of all TableRows in this hash key group, ordered by their
	 *         range key.
	 */
	public TableIterator<RangeTableRow<H, R, V>> range(H hashKey);
	 ...
	 * 
	 * @param hashKey      A key used group rows together.
	 * @param fromRangeKey The range key to use as the starting point. If an exact
	 *                     match doesn't exist for this hash/range pair, the nearest
	 *                     range key that is larger than this range key will be used
	 *                     as the starting point.
	 * @return An iterator of all remaining TableRows in this hash key group,
	 *         ordered by their range key.
	 */
	public TableIterator<RangeTableRow<H, R, V>> range(H hashKey, R fromRangeKey);

	/**
	 * Get all rows with a given hash key, and a range key that's greater than or
	 * equal to the "from" range key, and less than or equal to the "to" range key.
	 ...
	 *                     range key that is larger than this range key will be used
	 *                     as the starting point.
	 * @param toRangeKey   The range key to use as the end point (inclusive). If an
	 *                     exact match doesn't exist, the nearest range key that is
	 *                     less than this range key will be used as the end point
	 *                     (inclusive).
	 * @return An iterator of all remaining TableRows in this hash key group that
	 *         have a range key [fromRangeKey, toRangeKey).
	 */
	public TableIterator<RangeTableRow<H, R, V>> range(H hashKey, R fromRangeKey, R toRangeKey);
	 ...
	 *         range key.
	 */
	public TableIterator<RangeTableRow<H, R, V>> rangeReverse(H hashKey);
	 ...
	 * @param fromRangeKey The range key to use as the starting point. If an exact
	 *                     match doesn't exist for this hash/range pair, the nearest
	 *                     range key that is less than this range key will be used
	 *                     as the starting point.
	 * @return An iterator of all remaining TableRows in this hash key group,
	 *         reverse ordered by their range key.
	 */
	public TableIterator<RangeTableRow<H, R, V>> rangeReverse(H hashKey, R fromRangeKey);

	/**
	 * Get all rows with a given hash key, and a range key that's smaller than or
	 * equal to the "from" range key, and greater than or equal to the "to" range
	 * key in reverse iteration order. That is, the "from" range key is inclusive,
	 * and the "to" range key is inclusive.
	 * 
	 * @param hashKey      A key used group rows together.
	 * @param fromRangeKey The range key to use as the starting point. If an exact
	 *                     match doesn't exist for this hash/range pair, the nearest
	 ...
	 * @param toRangeKey   The range key to use as the end point (inclusive). If an
	 *                     exact match doesn't exist, the nearest range key that is
	 *                     larger than this range key will be used as the end point
	 *                     (inclusive).
	 * @return An iterator in reverse order of all remaining TableRows in this hash
	 *         key group that have a range key [fromRangeKey, toRangeKey).
	 */
	public TableIterator<RangeTableRow<H, R, V>> rangeReverse(H hashKey, R fromRangeKey, R toRangeKey);
	...
	/**
	 * Deletes all rows with a given hash key, and a range key that's greater than
	 * or equal to the "from" range key, and less than or equal to the "to" range
