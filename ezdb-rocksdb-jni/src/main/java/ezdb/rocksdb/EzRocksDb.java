...
public class EzRocksDb implements Db<byte[]> {
  ...
  public EzRocksDb(File root) {
    this(root, new EzRocksDbJniFactory());
  }

  public EzRocksDb(File root, EzRocksDbFactory factory) {
    this.root = root;
  ...
  public <H, R, V> RangeTable<H, R, V> getTable(String tableName, Serde<H> hashKeySerde, Serde<R> rangeKeySerde, Serde<V> valueSerde, Comparator<byte[]> hashKeyComparator, Comparator<byte[]> rangeKeyComparator) {
    synchronized (cache) {
      ...
      if (table == null) {
        table = new EzRocksDbTable<H, R, V>(new File(root, tableName), factory, hashKeySerde, rangeKeySerde, valueSerde, hashKeyComparator, rangeKeyComparator);
