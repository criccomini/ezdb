...
public final class TutorialTest {
	...
	public void tutorial1() throws IOException {
		...
		final File path = tmp.newFolder("tutorial1");
	 ...
	 */
	@Test
	@SuppressWarnings({ "ConvertToTryWithResources", "checkstyle:executablestatementcount" })
	public void tutorial2() throws IOException, InterruptedException {
		final File path = tmp.newFolder("tutorial2");
	...
	public void tutorial3() throws IOException {
		// As per tutorial1...
		final File path = tmp.newFolder("tutorial3");
		final Env<ByteBuffer> env = open(path, 10);
		final Dbi<ByteBuffer> db = env.openDbi(DB_NAME, MDB_CREATE);
		final ByteBuffer key = allocateDirect(env.getMaxKeySize());
		final ByteBuffer val = allocateDirect(700);
	...
	/**
	 * In this fourth tutorial we'll take a quick look at the iterators. These are a
	 * more Java idiomatic form of using the Cursors we looked at in tutorial 3.
	 *
	 * @throws IOException if a path was unavailable for memory mapping
	 */
	@Test
	public void tutorial4() throws IOException {
		// As per tutorial1...
		final File path = tmp.newFolder("tutorial4");
		final Env<ByteBuffer> env = open(path, 10);
		final Dbi<ByteBuffer> db = env.openDbi(DB_NAME, MDB_CREATE);
		...
		try (Txn<ByteBuffer> txn = env.txnWrite()) {
			...
			try (CursorIterable<ByteBuffer> it = db.iterate(txn, KeyRange.all())) {
				for (final KeyVal<ByteBuffer> kv : it) {
			...
			try (CursorIterable<ByteBuffer> it = db.iterate(txn, KeyRange.allBackward())) {
				for (final KeyVal<ByteBuffer> kv : it) {
					assertThat(kv.key(), notNullValue());
					assertThat(kv.val(), notNullValue());
				}
			...
			try (CursorIterable<ByteBuffer> it = db.iterate(txn, range)) {
				for (final KeyVal<ByteBuffer> kv : it) {
					assertThat(kv.key(), notNullValue());
					assertThat(kv.val(), notNullValue());
				}
	...
	public void tutorial5() throws IOException {
		// As per tutorial1...
		final File path = tmp.newFolder("tutorial5");
		final Env<ByteBuffer> env = open(path, 10);
